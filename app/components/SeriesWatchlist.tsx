...
interface seriesWatchlistProps {
  openInfoModal: (movie: WatchItem) => void; // Update to accept a movie parameter
}

const SeriesWatchlist: React.FC<seriesWatchlistProps> = ({ openInfoModal }) => {
  ...
  const series = watchList.filter(
    (item) => item.type === "Series" && item.status === "Watchlist"
  );
