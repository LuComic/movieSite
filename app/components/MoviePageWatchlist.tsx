...
interface moviePageWatchlistProps {
  openInfoModal: (movie: WatchItem) => void; // Update to accept a movie parameter
}

const MoviePageWatchlist: React.FC<moviePageWatchlistProps> = ({
  openInfoModal,
}) => {
  ...
  const movies = watchList.filter(
    (item) => item.type === "Movie" && item.status === "Watchlist"
  );
...
export default MoviePageWatchlist;
